                ...
                <div class="tab-content custom-scrollbar-5">
                    ...
                    <app-deposit-history>
                        <div class="tab-pane" id="nav-diposit-history" role="tabpanel">
                            <div class="admin-content-header-new">Withdray History</div>
...
<script>
	...
	function fetch_data()
	{
		...
		dataTable = $('#dataTable').DataTable({
			...
			"ajax": {
				url: base_url+"customeruser/withdraw_history_dt",
				type: "POST"
			},
	...
	function withdrawCancelByUser(id) {
		var url = '<?= base_url("customeruser/cancel_withdraw_by_user") ?>';
		swal({
			title: "Are you sure?",
			text: "Your current withdraw want to cancel",
			type: "warning",
			...
			if (isConfirm) {
				...
				swal("Transaction Cancelled", "Your Withdraw now pending)", "error");
				$('#dataTable').DataTable().destroy();
				fetch_data();
				return;
			}
