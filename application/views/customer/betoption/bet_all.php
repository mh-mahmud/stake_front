	...
	<div class="tab-pane-1" id="homeSports-tab" role="tabpanel">
		...
		foreach ($get_data as $val) : ?>
			<div class="single-match-result-accordion-game-header"
				...
				<a><?php echo get_sports_cat_name($val->sportscategory_id); ?><span class="icon">&nbsp&nbsp;<img
							width="25px"
							src="<?php echo get_admin_url() ?>assets/img/icon/<?php echo get_sports_cat_image($val->sportscategory_id); ?>"
							alt=""></span></a>
			...
			<?php
			$get_match_data = $this->db->query("SELECT * FROM matchname WHERE active_status=1 AND status=1 AND sportscategory_id='{$val->sportscategory_id}' ORDER BY serial ASC")->result();
			foreach ($get_match_data as $matchval) {
						...
						<div class="single-match-result">
							<div class="single-match-result-header" style="justify-content: center!important;">
								<a class="left-side" href="javascript:void(0);"
								   onclick="return live_bets_by_single_match(<?php echo $matchval->id; ?>);">
                                <span class="match-infos">

                                	<div class="left"><marquee behavior="scroll" direction="left" onmouseout="this.start();" onmouseover="this.stop();" scrollamount="1" scrolldelay="40" truespeed="truespeed"><?php echo $matchval->notification; ?></marquee></div>

                                    <span class="top-side">
                                        <span><?php echo $matchval->league_title; ?></span>
                                    </span>
                                    <span class="bottom-side">
                                        <span><?php echo $matchval->team1; ?>&nbsp;</span>
										<span class="live-match-title"> VS </span>
                                        <span class="">&nbsp;<?php echo $matchval->team2; ?></span>
                                    </span>
                                </span>
								</a>
								<div class="right-side">
									<div class="three_row" style="display: inline!important;">
										<?php if (!empty($matchval->youtubeLInk)): ?>
											<div class="rowColumn middle" onclick="youtube_link('<?= $matchval->youtubeLInk ?>');"><img src="<?= base_url("assets/img/youtube.png") ?>" style="width: 35px;height: 20px;cursor: pointer;"></div>
										<?php else: ?>
							...
							<?php
							$get_bet_data = $this->db->query("SELECT * FROM match_option WHERE match_id='{$matchval->id}' AND status=1 ORDER BY match_option_serial ASC")->result();
							$x = rand(10, 10000);
							$ii = $x + $matchval->id;
